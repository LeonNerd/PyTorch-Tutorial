...
class Net(nn.Module):  # 训练 ALexNet
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # 输入 32 * 32 * 3
            ...
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1),  # (32-3+2)/1+1 = 32
            nn.ReLU(),
        ...
        self.conv2 = nn.Sequential(  # 输入 32 * 32 * 16
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1, padding=1),  # (32-3+2)/1+1 = 32
            nn.ReLU(),

            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1),  # (32-3+2)/1+1 = 32
            nn.ReLU(),
        ...
        self.conv3 = nn.Sequential(  # 输入 16 * 16 * 6
            nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1),  # (16-3+2)/1+1 = 16
            nn.ReLU(),

            nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1),  # (16-3+2)/1+1 = 16
            nn.ReLU(),
        ...
        self.conv4= nn.Sequential(  # 输入 8 * 8 * 128
            nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1), # (8-3+2)/1+1 = 8
            nn.ReLU(),
        ...
        self.conv5 = nn.Sequential(  # 输入 4 * 4 * 64
            nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1), # (4-3+2)/1+1 = 4
            nn.ReLU(),
...
def train():
    ...
    optimizer = optim.Adam(net.parameters(), lr=0.001)  # 随机梯度下降
    iter = 0
    num = 1
    # 训练网络
