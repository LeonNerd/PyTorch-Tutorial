...
import numpy as np
# 主函数-创建文件夹
def judge_creat(file,folder):
    isExists = os.path.exists('H:/'+folder)     #生成文件夹
    if not isExists:
        os.makedirs('H:/'+folder)
    # os.chdir('H:/data/poles/20191104poles/xml/'+folder)                  #加入文件夹的路径
    # for JPG in file:
    #     print(JPG)
...
def txt_edit(files1,l):
    for txt in files1:
        ...
        q += 1
def edit_name():
    path = 'H:/data/poles/20191104poles/pict_flip/'
...
def video_process():
    import cv2
    import os
    save_path = r"H:/2/"  # 存储的位置
    path = r"H:/1/"  # 要截取视频的文件夹
                ...
                while success:
                    ...
                    if (c % timeF == 0):  # 每隔timeF帧进行存储操作
                        ...
                        cv2.imwrite(save_path + item.split(".")[0] + '_' + str(count) + ".jpg",
                                    image).encode('utf-8') # 存储图片的地址 以及对图片的命名
                        count += 1
                    c += 1
