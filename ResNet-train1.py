...
class ResNet34(nn.Module):
    def __init__(self, block):
        ...
        self.first = nn.Sequential(
            # 卷积层1：采用更小的3*3kernel，1stride，1padding，outmap：32-3+2*1 / 1 + 1，32*32
        ...
        self.layer2 = self.make_layer(block, 64, 128, 4, 2)  # 输出16*116
...
def main():
    ...
    # 定义数据集
    train_data = tv.datasets.CIFAR10(root='./Cifar-10', train=True, download=False, transform=transform)
    ...
    test_load = t.utils.data.DataLoader(test_data, batch_size=args.test_batch_size, shuffle=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #保存模型
        if args.save_model:
            t.save(net.state_dict(), modelPath)
